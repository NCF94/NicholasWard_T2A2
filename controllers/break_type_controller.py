...
from init import db
from models.break_type import BreakType, BreakTypeSchema, break_type_schema
...
def delete_break_type(break_id, type_id):
    ...
    if break_type:
        ...
        db.session.commit()
        return {'message': f'Break type {break_type.message} deleted successfully'}
    else:
...
def update_break_type(break_id, type_id):
    body_data = request.get_json()
    stmt = db.select(BreakType).filter_by(id=type_id)
    break_type = db.session.scalar(stmt) # comment from database that needs to be updated
    if break_type:
        break_type.message = body_data.get('break_type') or break_type.message
        db.session.commit()
